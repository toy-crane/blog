...
> **IMPORTANT:** Run all CLI commands using the project's package runner: `bunx --bun shadcn@latest`, `pnpm dlx shadcn@latest`, or `bunx --bun shadcn@latest` — based on the project's `packageManager`. Examples below use `bunx --bun shadcn@latest` but substitute the correct runner for the project.

## Current Project Context

```json
!`bunx --bun shadcn@latest info --json 2>/dev/null || echo '{"error": "No shadcn project found. Run shadcn init first."}'`
```
...
- **Never decode or fetch preset codes manually.** Pass them directly to `bunx --bun shadcn@latest init --preset <code>`.
...
9. **Switching presets** — Ask the user first: **reinstall**, **merge**, or **skip**?
   - **Reinstall**: `bunx --bun shadcn@latest init --preset <code> --force --reinstall`. Overwrites all components.
...
bunx --bun shadcn@latest init --defaults  # shortcut: --template=next --preset=base-nova
